...
def prediction_page():
    ...
    ApplicantIncome = st.slider("💰 Applicant Income", min_value=1000, max_value=50000, step=1000, value=5000)
    CoapplicantIncome = st.slider("🤝 Coapplicant Income", min_value=0, max_value=50000, step=1000, value=0)
    LoanAmount = st.slider("🏦 Loan Amount", min_value=50, max_value=700, step=10, value=150)
    st.info("💡 **Note:** The model was trained on historical loans between $50-$700. Keeping the slider in this range ensures accurate predictions during demos.")
